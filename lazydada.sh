...
##############################################
#
# Lazydada by Technical Dada
# Just made this for when My finger Cracked
# Installs quite a few extras to a Fresh Kali:)
# questions comments or request me at technicaldada1@gmail.com
# for my tech blog https://technicaldada4us.blogspot.in
#
##############################################
clear
...
if [ $UID -ne 0 ]; then
    echo -e "\033[31 This program must be run as root.This will probably fail.\033[m"
    sleep 3
fi

###### Install script if not installed
if [ ! -e "/usr/bin/lazydada" ];then
	echo "Lazydada is not installed. Do you want to install it ? (Y/N)"
	read install
	if [[ $install = Y || $install = y ]] ; then
		cp -v $0 /usr/bin/lazydada
		chmod +x /usr/bin/lazydada
		echo "LazyDada should now be installed. Launching it !"
		sleep 3
		/usr/bin/lazydada
		exit 1
	else
...
if [[ "$silent" = "1" ]];then
  ...
  #store last version number to variable
	last_version=$(curl --silent -q https://raw.githubusercontent.com/jlevitsk/lazykali/master/version)
...
Do you want to update to \033[32m$last_version\033[m? (Y/N)"
		...
		if [[ $update = Y || $update = y ]];then
			echo "[+] Updating Lazydada..."
			wget -q https://raw.githubusercontent.com/jlevitsk/lazykali/master/lazykali.sh -O $0
			chmod +x $0
			echo "[-] LazyDada updated !"
			if [[ $0 != '/usr/bin/yamas' && $ask_for_install = 'y' ]];then
				echo -e "Do you want to install it so that you can launch it with \"lazydada\" ?"
				read install
				if [[ $install = Y || $install = y ]];then #do not proceed to install if using installed version : updating it already "installed" it over.
					...
					echo "Script should now be installed, launching lazydada !"
					sleep 3
					lazykali
					exit 1
				else
...
Forked lazidada code by Technical Dada - https://technicaldada4us.blogspot.com
...
\033[31m#######################################################\033[m
                Metasploit Services
\033[31m#######################################################\033[m"
select menusel in "Start Metasploit" "Reset Metasploit" "Back to Main Menu" "Exit"; do
case $menusel in
	"Start Metasploit")
...
function installhackpack {
	...
	if [[ $install = Y || $install = y ]] ; then
		cd /tmp
		wget https://raw.githubusercontent.com/jlevitsk/lazykali/master/hackpack.tar.gz
...
echo -e "\e[31m *                      By Technical Dada                         *\e[0m"
echo -e "\e[31m *************************************************************\e[0m"
...
######## Install Torghost
function installtorghost {
	echo "This will install Tor ghost. Do you want to install it ? (Y/N)"
	read install
	if [[ $install = Y || $install = y ]] ; then
...
select menusel in "WiFiPhisher" "Hackpack" "Google Chrome" "Flash" "Tor ghost" "Smbexec" "Xssf" "Ettercap 0.76" "AngryIP Scanner" "Terminator" "Xchat" "Unicornscan" "Nautilus Open Terminal" "Simple-Ducky" "Subterfuge" "Ghost-Phisher" "Java" "Install All" "Back to Main Menu" "Exit"; do
case $menusel in
	"WiFiPhisher")
		installwifiphisher
		pause
